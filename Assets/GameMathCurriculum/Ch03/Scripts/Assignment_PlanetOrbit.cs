...
public class Assignment_PlanetOrbit : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        planetWorldPos = planetMatrix.MultiplyPoint3x4(new Vector3(planetOrbitRadius, 1f, 0f));
        transform.position = planetWorldPos;

        Matrix4x4 satelliteMatrix = Matrix4x4.TRS(planetWorldPos, Quaternion.Euler(0f, Time.time * satelliteOrbitSpeed, 0f), Vector3.one );
        satelliteWorldPos = satelliteMatrix.MultiplyPoint3x4(new Vector3(satelliteOrbitRadius, 0f, 0f));
