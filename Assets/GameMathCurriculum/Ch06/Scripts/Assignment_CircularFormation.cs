...
public class Assignment_CircularFormation : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < currentUnitCount; i++)
        {
            ...
            Quaternion rotation = Quaternion.AngleAxis((360f / currentUnitCount) * i, leader.up);
            Vector3 unitPosition = rotation * new Vector3(formationRadius, 0f, 0f) + leader.position;
            units[i].position = unitPosition;
            units[i].rotation = Quaternion.LookRotation(units[i].position - leader.position);
